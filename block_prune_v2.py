...
# model
model = ResNet_v2_block(depth=config.depth, block_pruning=block_pruning,
...
def build_new_model():
    new_model = ResNet_v2_block(depth=config.depth, block_pruning=block_pruning,
    ...
    for layer in range(len(old_modules)):
        if isinstance(old_modules[layer], Bottleneck_block):
            index += 1
    ...
    for layer, module in enumerate(new_model.modules()):
        if isinstance(module, Bottleneck_block):
            ib += 1
