...
def get_args():
    ...
    parser.add_argument('--save_path', default='./exp/finetune', type=str, metavar='PATH',
                        help='path to save dir (default: none)')
    parser.add_argument('--finetune', default='./exp/block_prune/pruned.pth.tar', type=str, metavar='PATH',
...
def main():
    ...
    #m, n = torch.sort(model.alpha)
    #num_block = model.alpha.data.shape[0]
    #thre = m[int(0.4*num_block)].data
    #tmp = model.alpha.data
    #model.gate.data = torch.gt(tmp, thre)

    model.set_test_gates()
    model.gate.requires_grad = False
    print(model.gate.data)
    for param_group in base_optimizer.param_groups:
        param_group['lr'] = 0.001
    for epoch in range(config.start_epoch, config.start_epoch+10):
